...
def fetch_with_cloudvolume(source, ids, decim_fraction, input_json_dir):
    ...
    try:
        ...
        print(f"Error: fetching from source '{source}' failed: {str(e)}")
...
def fetch_directly(source, mesh_info, ids, lod, decim_fraction, input_json_dir):
            ...
            for i in range(len(neuron_sources)):
                ...
                if is_ng_source(source):
                    ...
                    if is_cloudvolume_accessible(mesh_info):
                        fetch_with_cloudvolume(source, neuron_ids[i], args.decim_fraction, input_json_dir)
                    else:
                        fetch_directly(source, mesh_info, neuron_ids[i], args.lod, args.decim_fraction, input_json_dir)
