...
<!-- CHUNK START -->
<!-- STEP: 1A -->
The neuron `"source"` URL for the Janelia FlyEM hemibrain v1.2 (or the FlyEM hemibrain, or the hemibrain) is: `"https://hemibrain-dvid.janelia.org/api/node/31597/segmentation_meshes"`.
<!-- CHUNK END -->
<!-- CHUNK START -->
<!-- STEP: 1A -->
The neuron `"source"` URL for the Janelia FlyEM MANC (or the FlyEM MANC, or the MANC, or the FlyEM VNC, or the VNC) is: `"https://manc-dvid.janelia.org/api/node/v1.0/segmentation_meshes"`.
<!-- CHUNK END -->
<!-- CHUNK START -->
<!-- STEP: 1A -->
...
<!-- CHUNK START -->
<!-- STEP: 2 -->
When the any of the neurons (even only one element of a `"source"` array) come from the Janelia FlyEM MANC (or the FlyEM MANC, or the MANC, or the FlyEM VNC, or the VNC), `"https://manc-dvid.janelia.org/api/node/v1.0/segmentation_meshes"`, then before the `"animation"` array there should be a `"lightRotationY"` key to give the three area lights an orientation that matches this data set: 
```json
{
  "neurons": {
    "source": "https://manc-dvid.janelia.org/api/node/v1.0/segmentation_meshes",
...
<!-- CHUNK START -->
<!-- STEP: 2 -->
When the any of the neurons (even only one element of a `"source"` array) come from the FlyWire FAFB (or FlyWire), `"precomputed://gs://flywire_v141_m630"`, then before the `"animation"` array there should be a `"lightRotationX"` key to give the three area lights an orientation that matches this data set: 
```json
{
  "neurons": {
    "source": "precomputed://gs://flywire_v141_m630",
...
and whose second element is an object, the command arguments.
<!-- CHUNK END -->
...
<!-- CHUNK START -->
<!-- STEP: 2 -->
Show the neurons in group `"Y"` by making them fade from alpha `0` (fully transparent or invisible) to alpha `1` (fully opaque or fully visible), with the fading taking `2` seconds per the `"duration"`:
```json
{
  "animation": [
...
<!-- CHUNK START -->
<!-- STEP: 2 -->
With the `"scale"` value being `0.5`, the camera is half again as far from the neurons in group `"M"` as for a normal framing (so the neurons appear 2 times bigger). The framing happens instantaneously, at the current time:
```json
{
  "animation": [
...
<!-- CHUNK START -->
<!-- STEP: 2 -->
With the `"scale"` of `3`, the camera ends up 3 times farther away from the neurons in group `"E"` than for a normal framing (so the neurons appear 1/3 as big), and the movement of the camera takes `5` seconds, per the `"duration"` argument:
```json
{
  "animation": [
...
<!-- CHUNK START -->
<!-- STEP: 2 -->
Orbit the camera halfway around (i.e., `180` degrees, per the `"endingRelativeAngle"` argument) what the camera is pointed at (centered on) currently. Here, it is centered on neuron group `"C"` because of the `"frameCamera"` before the `"orbitCamera"`. The orbit takes `4` seconds, per the `"duration"` argument. The rotation axis is `"z"` by default:
```json
{
  "animation": [
...
<!-- CHUNK START -->
<!-- STEP: 2 -->
Orbit the camera `-80` degrees, per the `"endingRelativeAngle"` argument, and move the camera out (i.e., farther from what it is orbiting around) as the orbiting proceeds, so it ends up twice as far away as it started, per the `"scale"` argument's value, `2`:
```json
{
  "animation": [
...
<!-- CHUNK START -->
<!-- STEP: 2 -->
When the neuron mesh source (or one of the sources in a `"source"` array) is the Janelia FlyEM MANC (or the FlyEM MANC, or the MANC, or the FlyEM VNC, or the VNC), `"https://manc-dvid.janelia.org/api/node/v1.0/segmentation_meshes"`, the `"animation"` array *must* start with an `"orbitCamera"` of angle `180` about the `"y"` axis and no `"duration"`. All other commands (`"frameCamera"`, `"fade"`, `"setValue"`, etc.) *must* follow this first command:
```json
{
  "animation": [
    ["orbitCamera", {"axis": "y", "endingRelativeAngle": 180}]
...
<!-- CHUNK START -->
<!-- STEP: 2 -->
When the any of the neurons (even only one element of a `"source"` array) come from the FlyWire FAFB (or FlyWire), `"precomputed://gs://flywire_v141_m630"`, the `"animation"` array *must* start with an `"orbitCamera"` of `-90` about the `"x"` axis and no `"duration"`. All other commands *must* follow this first command:
```json
{
  "animation": [
    ["orbitCamera", {"axis": "x", "endingRelativeAngle": -90}]
...
<!-- CHUNK START -->
<!-- STEP: 2 -->
For an ROI, the exponent controls the silhoutte line rendering: a lower exponent (e.g., `2`, `3`) makes the ROI appear heavier and more prominent, while a higher exponent (e.g., `5`, `6`) makes the ROI appear lighter and less conspicuous. Set the exponents of the ROIs in group `"R"` to `6`, per the `"exponent"` argument, from the current time forward:
```json
{
  "animation": [
...
<!-- CHUNK START -->
<!-- STEP: 2 -->
Fade for `1` second. Wait (pause) for `5` seconds, then frame for `2` seconds. Note there must be one `"advanceTime"` matching the `"duration"` of the fading and then another `"advanceTime"` for the `5` seconds of the waiting (pausing):
```json
{
  "animation": [
